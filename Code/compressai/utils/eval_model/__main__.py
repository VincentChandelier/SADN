...
@torch.no_grad()
def inference(model, x, f, outputpath):
    x = x.unsqueeze(0)
...
@torch.no_grad()
def inference_entropy_estimationshow(model, x, f, outputpath):
    x = x.unsqueeze(0)
    ...
    start = time.time()
    y_hat, likelihoods, scales_hat, means_hat, out_net = model.compressshow(x_padded)
    elapsed_time = time.time() - start

    y_mat = y_hat.squeeze(0).permute(1, 2, 0).cpu().numpy()
    y_mat = np.array(y_mat)
    io.savemat(imgPath[:-4]+"_y.mat", {'y_mat': y_mat})

    likelihoods_mat = likelihoods.squeeze(0).permute(1, 2, 0).cpu().numpy()
    likelihoods_mat = np.array(likelihoods_mat)
    io.savemat(imgPath[:-4] + "_likelihoods.mat", {'likelihoods_mat': likelihoods_mat})

    scales_mat = scales_hat.squeeze(0).permute(1, 2, 0).cpu().numpy()
    scales_mat = np.array(scales_mat)
    io.savemat(imgPath[:-4] + "_scales.mat", {'scales_mat': scales_mat})

    means_mat = means_hat.squeeze(0).permute(1, 2, 0).cpu().numpy()
    means_mat = np.array(means_mat)
    io.savemat(imgPath[:-4] + "_means.mat", {'means_mat': means_mat}
               )
...
def eval_model(model, filepaths, entropy_estimation=False, half=False, outputpath='Recon'):
    ...
    for f in filepaths:
        ...
        if not entropy_estimation:
            if half:
                model = model.half()
                x = x.half()
            rv = inference(model, x, f, outputpath)
        else:
...
def main(argv):
    ...
    for run in runs:
        ...
        if args.cuda and torch.cuda.is_available():
            model = model.to("cuda")
        metrics = eval_model(model, filepaths, args.entropy_estimation, args.half, args.output_path)
        for k, v in metrics.items():
            results[k].append(v)
